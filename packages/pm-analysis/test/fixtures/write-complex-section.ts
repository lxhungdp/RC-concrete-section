/**
 * Writes the neutral complex-section regression example, then re-parses it through
 * `parseProjectDocument` so a file that cannot be imported is never produced.
 *
 * Run: npm run fixture:complex-section-json
 */
...
const target = resolve(process.cwd(), 'docs/examples/realistic-sections/KDS-REAL-05-complex-stress-strain.pm-project.json')
