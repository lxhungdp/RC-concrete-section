...
The standalone kernel can optionally measure/refine normalized chord error in both direction and neutral-axis-depth coordinates. The application production profile instead supplies the fixed 27-station / 36-direction grid. For a selected reported inverse capacity, use that surface only as a branch/initial-state seed and pass the exact evaluator to `solveProportionalRayCapacity(...)`; its damped solve is the authoritative result.
