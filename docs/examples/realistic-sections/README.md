...
These schema-v1 projects provide current UI and regression examples.
...
| `KDS-REAL-05-complex-stress-strain.pm-project.json` | 1500 x 1200 chamfered section, two voids, symmetric D32 reinforcement | 18 | 1.292% | 54 mm |

The first four equivalent-block examples are generated together and verify that every bar centre is in concrete, clear cover is at least 60 mm, clear
...
The stress-strain example is a neutral deterministic regression input, not an external comparison
oracle. Regenerate and parse-check it with `npm run fixture:complex-section-json`.
