...
| Fixed-grid utilization screening policy | `packages/pm-results` classification contract, selected by the mechanics owner | [`engineering/05-loadings-analysis-results-and-reports.md`](engineering/05-loadings-analysis-results-and-reports.md) | stress-strain uses its measured 2% regression envelope; equivalent-block Fixed is fail-closed until it has a validated bound |
